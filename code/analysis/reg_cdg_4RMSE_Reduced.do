...
use treated_hosp_weathter_full_cdg0, clear
	...
	cap erase "$tables/AppendixTable/AppendixTableA10.xls"
	cap erase "$tables/AppendixTable/AppendixTableA10.txt"
	...
	foreach prefix in "l_" {
		foreach suffix in   `w'_pop  {
			foreach sample in "1"  { 
				...
				outreg2 using "$tables/AppendixTable/AppendixTableA10.xls" ,append dec(3) keep(CUM_elow_CB* CUM_ehigh_CB*)
                   }
				}
			}
		}
