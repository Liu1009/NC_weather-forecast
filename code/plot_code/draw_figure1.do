	...
	replace y_Expense_total=0.137*3*6 if _n==1 
	replace y_Expense_total=0*6 if _n==2
	replace y_Expense_total=0.046*3*6 if _n==3
	
	replace y_visit=0.073*3*6 if _n==1
	replace y_visit=0*6 if _n==2
	replace y_visit=0.027*3*6 if _n==3
...
twoway  (rarea ub lb time if type==2 & var=="expense" ,color("112 160 255 %60") lc(blue%60))  ///
 ...
 (scatter coef time if time==3 & var=="expense",color(black%60)  yline(0) xtitle("Forecast Error (°C)")  ytitle("Medical Expenses") xlabel(0 "≤-3" 1 "-2" 2 "-1" 3 "0" 4 "1" 5 "2" 6 "≥3", tposition(inside)) ylabel(-1 0 1 2 3, grid tposition(inside))) 
...
twoway  (rarea ub lb time if type==2 & var=="visit" ,color("112 160 255 %60") lc(blue%60))  ///
 ...
 (scatter coef time if time==3 & var=="visit",color(black%60)  yline(0) xtitle("Forecast Error (°C)")  ytitle("Healthcare visit") xlabel(0 "≤-3" 1 "-2" 2 "-1" 3 "0" 4 "1" 5 "2" 6 "≥3", tposition(inside)) ylabel(-1 0 1 2 3, grid tposition(inside)))
