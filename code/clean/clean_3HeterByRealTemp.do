...
use hosp_weathter_full_cdg0s, clear
	...
	*- Generate variables of forecast erros for parametric estimation
			...
			bysort city_id: egen decile = xtile(r_TempHigh), nq(10)
			
			forvalues i = 1/10 {
				gen r_TempHigh_q`i' = (decile == `i')
			...
			forval j=0(1)`periods' {
				...
				bys city_id groups (date_ymd): gen dif_TempHigh_L`j'=dif_TempHigh[_n-`j']
					...
					forval bins =1(1)10 {
						...
						gen e_low_L`j'_B`bins'= e_low_L`j' *(r_TempHigh_q`bins'==1) 
						gen e_high_L`j'_B`bins'= e_high_L`j' *(r_TempHigh_q`bins'==1) 
						gen is_high_L`j'_B`bins'= is_high_L`j' *(r_TempHigh_q`bins'==1) 
					}
			...
			*translate to average cummulative effect
			...
			cap replace  L0_rT_CB`bins'=r_TempHigh_L0_B`bins'
			...
			cap replace  L`j'_rT_CB`bins'= r_TempHigh_L`j'_B`bins' -L0_rT_CB`bins'
